...
// 1. Fix roster Raw Data loop
const rosterStr = `                if (name && id) {
                    idToNameMap[id] = name.toString().trim();
                      nameToIdMap[name.toString().trim()] = id.toString().trim();
                      enrolledGameIds.add(id.toString().trim());
                  }`;

const rosterFix = `                if (name && id) {
                    idToNameMap[id] = name.toString().trim();
                      nameToIdMap[name.toString().trim()] = id.toString().trim();
                  }`;
                  
content = content.replace(rosterStr, rosterFix);


// 2. Fix Alt Account Button
const altBtnStr = `<div style="display:flex; flex-direction:column; gap:5px; align-items:flex-end;">
...
content = content.replace(altBtnStr, altBtnFix);


// 3. Fix Main Account Button
const mainBtnStr = `const chiefName = currentUser.name || idToNameMap[currentUser.gameId] || "Unknown Chief";
        contentHtml = \`
...
const mainBtnFix = `const chiefName = currentUser.name || idToNameMap[currentUser.gameId] || "Unknown Chief";
...
content = content.replace(mainBtnStr, mainBtnFix);
